'''
Goal of Prophet microservice:
1. Prophet microservice will accept the GitHub data from Flask microservice and will forecast the data for next 1 year based on past 30 days
...
def forecast():
    ...
    Prophet_GENERATED_IMAGE_NAME = "all_issues_data_" + type + "_"+ repo_name + ".png"
    Prophet_GENERATED_IMAGE_NAME = BASE_IMAGE_PATH + Prophet_GENERATED_IMAGE_NAME

    # Add your unique Bucket Name if you want to run it local
    BUCKET_NAME = os.environ.get(
        'BUCKET_NAME', 'spmprophetbucket')
    ...
    plt.savefig(LOCAL_IMAGE_PATH + Prophet_GENERATED_IMAGE_NAME)
    ...
    new_blob = bucket.blob(Prophet_GENERATED_IMAGE_NAME)
    new_blob.upload_from_filename(
        filename=LOCAL_IMAGE_PATH + Prophet_GENERATED_IMAGE_NAME)
    

    # Construct the response
    json_response = {
        "prophet_generated_image_url": Prophet_GENERATED_IMAGE_NAME
    }
    # Returns image url back to flask microservice
    return jsonify(json_response)


# Run LSTM app server on port 2000
