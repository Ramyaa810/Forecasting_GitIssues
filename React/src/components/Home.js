      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        ...
        {loading ? (
          ...
          <div>
            ...
            />
            <LineCharts
              title={`Issues for all repos in last 2 year repo wise`}
            ...
            <BarCharts
              ...
              data={githubRepoData?.created}
            />     
             {/* Render barchart component for a stars in all repo*/}      
            <BarCharts
              title={`Stars for all repos in last 2 year`}
              data={githubRepoData?.stars}
            />
            {/* Render barchart component for a forks in all repo*/}
            <BarCharts
            ...
            <BarCharts
              ...
              data={githubRepoData?.closedWeek}
            />
            {/* Render stackbarchart component for a created and closed issues for a selected repositories*/}
            <StackBarCharts
              title={`Created and Closed Issues for ${repository.value} in last 2 year`}
              data={githubRepoData?.created}
              data1={githubRepoData?.closed}
            />
            ...
            <Typography component="h4">
                  The day of the week with Maximum issue created for the repository is <span style={{fontWeight: 'bold'}}>{githubRepoData?.max_created_day}</span> 
              </Typography>
            <LineCharts
              title={`Number of Issues created for particular week days`}
              data={githubRepoData?.DailyCreated}
            />
            <div>
            <Typography component="h4">
                  The day of the week with Maximum issue closed for the repository is<span style={{fontWeight: 'bold'}}>{githubRepoData?.max_closed_day}</span> 
              </Typography>
            <LineCharts
              title={`Number of Issues closed for particular week days`}
              data={githubRepoData?.DailyClosed}
            />
            <Typography component="h4">
                  The month of the year which has Maximum issue closed for the repository is<span style={{fontWeight: 'bold'}}> {githubRepoData?.max_closed_month}</span> 
              </Typography>
            <LineCharts
              title={`Number of Issues closed for particular month`}
              data={githubRepoData?.MonthWiseClosed}
            />
            </div>
            </div>
            ...
            <div>
              ...
              <div>
                ...
                {/* Render the all issues data image for pulls*/}
                <img
            ...
            <Divider
              ...
              />
              <Typography variant="h5" component="div" gutterBottom>
                Timeseries Forecasting using Prophet based on past month
              </Typography>
              <div>
                <Typography component="h4">
                  Forecasting of Created Issues
                </Typography>
                {/* Render the forecasting of Created Issues  */}
                 {/* <img
                  src="env\images\angular_plot_1.png"
                  alt={"Forecasting of Created Issues"}
                  loading={"lazy"}
                />
              </div>
            </div> */}
